...
CAP1188Channel::CAP1188Channel() : cap1188(nullptr),pin(-1){};
CAP1188Channel::~CAP1188Channel(){};

void CAP1188Channel::initialise(CAP1188& cap1188_, int pin_){
    cap1188 = &cap1188_;
    pin = pin_;
}

bool CAP1188Channel::isPinTouched(){
    uint8_t touchedPins = cap1188->touched();
    return touchedPins & (1 << (pin-1));
}

int CAP1188Channel::pinDelta(){
    return cap1188->deltaCount(pin);
}

int CAP1188Channel::getThreshold(){
    std::array<uint8_t, 8> allThresholds = cap1188->getThresholds();
    return allThresholds[pin-1];
}
...
CAP1188::CAP1188(I2CBus& bus,
    ...
    :   bus_(bus),
        ...
        for (int i = 0; i < 8; i++) channels[i].initialise(*this, i);
}
...
bool CAP1188::initialise()
{
    ...
    uint8_t mid = 0;
    bus_.readBlock(address_, CAP1188_REG_MANUFACTURER_ID, &mid, 1);
    if (mid != CAP1188_MANUFACTURER_ID_VALUE)
        return false;
    uint8_t pid = 0;
    bus_.readBlock(address_, CAP1188_REG_PRODUCT_ID, &pid, 1);
    if (pid != CAP1188_PRODUCT_ID_VALUE)
        return false;

    // Enable/Disable Channels
    bus_.writeBlock(address_, CAP1188_REG_SENSOR_ENABLE, 
    &enabledChannels_, 1);
    bus_.writeBlock(address_, CAP1188_REG_INTERRUPT_ENABLE,
    &interruptEnabledChannels_, 1);
    
    // LED Linking on, so Sensor trips LED on board
    uint8_t setHigh = 0;
    bus_.writeBlock(address_, CAP1188_REG_LED_LINKING,
    &setHigh, 1);
...
void CAP1188::handleDataReady()
{
    ...
    uint16_t delta = channels[0].pinDelta();
...
int CAP1188::touched(){
    ...
    uint8_t current = 0;
    bus_.readBlock(address_, CAP1188_REG_MAIN_CTRL, &current, 1);
    current = current * ~0x01;
    bus_.writeBlock(address_, CAP1188_REG_MAIN_CTRL, &current, 1);
    //Now read the sensor status
    uint8_t touchVals = 0;
    bus_.readBlock(address_, CAP1188_REG_SENSOR_STATUS, &touchVals, 1);
    return touchVals;
}
...
uint8_t CAP1188::sensitivityGetter(){
    ...
    bus_.readBlock(address_, CAP1188_REG_SENSITIVITY, &sensitivity, 1);
    return sensitivity;
}
void CAP1188::sensitivitySetter(uint8_t sensitivityVal){
    // Bits 4-6 control the sensitivity so a shift is needed
    sensitivityVal <<= 4;
    // Get what's in the register
    uint8_t current = CAP1188::sensitivityGetter();
    // Clear bits 4-6 and preserve the others
    current &= 0x8F;
    uint8_t newVal = current | sensitivityVal;
    bus_.writeBlock(address_, CAP1188_REG_SENSITIVITY, &sensitivityVal, 1);
}

uint8_t CAP1188::getAveraging(){
    uint8_t samplesPerAverageIx = 0;
    bus_.readBlock(address_, CAP1188_REG_AVG_SAMP_RATE, &samplesPerAverageIx, 1);
...
void CAP1188::setAveraging(uint8_t averagingVal) {
    ...
    uint8_t reg = 0;
    bus_.readBlock(address_, CAP1188_REG_AVG_SAMP_RATE, &reg, 1);
    ...
    // Write to register
    bus_.writeBlock(address_, CAP1188_REG_AVG_SAMP_RATE, &avg, 1);
}
...
void CAP1188::setSampleTime(std::string newSampleTime){
    ...
    bus_.writeBlock(address_, CAP1188_REG_AVG_SAMP_RATE, &newReg, 1);
}
...
void CAP1188::setCycleTime(std::string newCycleTime){
    ...
    reg &= 0xFC;
    // Get index
    uint8_t timeIx = std::distance(CAP1188_SAMP_TIME_OPTIONS.begin(), it);
    uint8_t newReg = reg | timeIx;
    bus_.writeBlock(address_, CAP1188_REG_AVG_SAMP_RATE, &newReg, 1);
}
...
std::array<uint8_t, 8> CAP1188::getThresholds() {
    ...
    bus_.readBlock(address_, CAP1188_REG_THRESH_CS1, block.data(), 8);
    return block;
}
void CAP1188::setThresholds(std::array<uint8_t, 8> newThresholds){
    ...
    bus_.writeBlock(address_, CAP1188_REG_THRESH_CS1, newThresholds.data(), 8);
}

// Take a mask of pins ie 00100001 for Pins 1 & 6
// Forces reset of Calibration value
void CAP1188::recalibratePins(uint8_t pins){
    bus_.writeBlock(address_, CAP1188_REG_CALIB_ACTIVATE, 
    &pins, 1);
}

uint8_t CAP1188::deltaCount(int pin){
        // Pin Check
        if (pin < 1 || pin > 8)
            throw std::out_of_range("Pin must be integer 1-8");

        // Grab from pin's delta count register
        uint8_t delta;
        switch (pin){
            case 1: bus_.readBlock(address_, CAP1188_REG_DELTA_COUNT_CS1, &delta, 1); break;
            case 2: bus_.readBlock(address_, CAP1188_REG_DELTA_COUNT_CS2, &delta, 1); break;
            case 3: bus_.readBlock(address_, CAP1188_REG_DELTA_COUNT_CS3, &delta, 1); break;
            case 4: bus_.readBlock(address_, CAP1188_REG_DELTA_COUNT_CS4, &delta, 1); break;
            case 5: bus_.readBlock(address_, CAP1188_REG_DELTA_COUNT_CS5, &delta, 1); break;
            case 6: bus_.readBlock(address_, CAP1188_REG_DELTA_COUNT_CS6, &delta, 1); break;
            case 7: bus_.readBlock(address_, CAP1188_REG_DELTA_COUNT_CS7, &delta, 1); break;
            case 8: bus_.readBlock(address_, CAP1188_REG_DELTA_COUNT_CS8, &delta, 1); break;
            default: delta = 0; break; // safety fallback
            }
...
{
    uint8_t status = 0;
    bus_.readBlock(address_, CAP1188_REG_MAIN_CTRL, &status, 1);
    status &= ~CAP1188_MAIN_STBY_BIT;
    bus_.writeBlock(address_, CAP1188_REG_MAIN_CTRL, &status, 1);
}
